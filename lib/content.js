...
export const DEFAULT_CONTENT = {
  // Template visual do site (galeria de temas). "classico" = o layout atual, no ar.
  template: "classico",
  brand: {
    name: "Paula",
  ...
  // Rastreamento / analytics. Vazio = nada é carregado (cada cliente usa o próprio Pixel).
  tracking: {
    metaPixelId: "", // ID do Pixel da Meta (Facebook/Instagram). Só números.
    // Token da API de Conversões (CAPI). SEGREDO: usado só no servidor, nunca é enviado
    // ao site público. Fallback para a variável de ambiente META_CAPI_TOKEN.
    metaCapiToken: "",
    // Webhook (Google Apps Script) que recebe cada evento e escreve numa planilha do Google.
    // Usado só no servidor; fallback para a env SHEET_WEBHOOK_URL. Vazio = desligado.
    sheetWebhookUrl: "",
    // Código de teste da aba "Eventos de teste" da Meta (ex.: TEST12345). TEMPORÁRIO:
    // só para validar a CAPI. Deixe vazio em produção. Fallback env META_TEST_EVENT_CODE.
    testEventCode: "",
  },
