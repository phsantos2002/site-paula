...
// Etapa do funil de captação (acompanhamento interno no /admin).
export const ETAPAS = ["captado", "fotos_tratadas", "cadastrado", "publicado"];
...
function normalize(list) {
  return (Array.isArray(list) ? list : []).map((p, i) => {
    ...
    return {
      ...
      // etapa: funil interno. Legado sem o campo => "publicado" (já estão no ar).
      etapa: ETAPAS.includes(p.etapa) ? p.etapa : "publicado",
